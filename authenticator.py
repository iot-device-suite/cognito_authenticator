...
class AWSSRP(object):
    ...
    def create_auth_challenge(self, client=None):
        boto_client = self.client or client
        auth_params = self.get_auth_params()
        response = boto_client.admin_initiate_auth(
            UserPoolId=USER_POOL_PROVIDER_ID,
    ...
    def verify_challenge_response(self, client_response, client=None):
        boto_client = self.client or client
        tokens = boto_client.admin_respond_to_auth_challenge(
            UserPoolId=USER_POOL_PROVIDER_ID,
            ClientId=self.client_id,
            ChallengeName=self.PASSWORD_VERIFIER_CHALLENGE,
            ChallengeResponses=client_response
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Get a JSON Webtoken for the OSB CCES IoT Device Manager"
    )

    parser.add_argument(
        "--username",
        type=str,
        help="The E-Mail that you use for the IoT Device Manager"
    )

    parser.add_argument(
        "--password",
        type=str,
        help="Your Password in single quote notation"
    )
